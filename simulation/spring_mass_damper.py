...
def dlsim(A: np.ndarray, B: np.ndarray, C: np.ndarray, D: np.ndarray, u: np.ndarray, x0: np.ndarray) -> np.ndarray:
...
class SMD:
    ...
    def __init__(self, k: float, b: float, m: float, dt: float, output_type: str = "position"):
    ...
    def get_data_batch(self, batch_size: int, num_previous_obs: int = 1, num_previous_acts: int = 0) -> List[Tuple[Tuple[List[np.ndarray], ...], ...]]:
