...
def get_camarillo_matrices(cable_positions: List[Tuple[Tuple[float, ...], ...]],
    ...
    for segment_num in range(num_segments):
        ...
        K_m_inv_diag_list[3*segment_num:3*segment_num+3] = [1/segment_stiffness_vals[segment_num][1], 1/segment_stiffness_vals[segment_num][1], 1/segment_stiffness_vals[segment_num][0]]
    
        # Form D matrix for segment
        row_1 = np.array([-position[0] for position in cable_positions[segment_num]], dtype=float).reshape((1,-1))
        row_2 = np.array([-position[1] for position in cable_positions[segment_num]], dtype=float).reshape((1,-1))
        ...
        L_0_diag_list[3*segment_num:3*segment_num+3] = [segment_lengths[segment_num]] * 3

        # Form Lt diagonal matrix list        
        cables_so_far = sum(cables_per_segment[:segment_num])
        num_cables = cables_per_segment[segment_num]
        L_t_diag_list[cables_so_far:cables_so_far + num_cables] = [additional_cable_length + sum(segment_lengths[:segment_num+1])] * num_cables

        # Form Kt^-1 diagonal matrix list
        K_t_inv_diag_list[cables_so_far:cables_so_far + num_cables] = [1/x for x in cable_stiffness_vals[segment_num]]
    ...
    for i in range(num_segments):
        for segment_num in range(i, num_segments):
            cables_so_far = sum(cables_per_segment[:segment_num])
            num_cables = cables_per_segment[segment_num]
            D[3*i:3*i+3, cables_so_far:cables_so_far + num_cables] = D_matrix_list[segment_num]

    return D, K_m_inv, L_0, L_t, K_t_inv

def camarillo_constant_curvature_no_slack(delta_lengths: List[Tuple[float, ...]],
    ...
    y = np.concatenate([np.array(x, dtype=float).reshape((-1,1)) for x in delta_lengths], axis=0)

    # Form matrices used for forward and inverse kinematics
    D, K_m_inv, L_0, L_t, K_t_inv = get_camarillo_matrices(cable_positions, segment_stiffness_vals, cable_stiffness_vals, segment_lengths, additional_cable_length, cables_per_segment)
    
    # Form C_m and C_m^-1
    C_m = np.matmul(np.matmul(np.matmul(np.transpose(D), L_0), K_m_inv), D) + np.matmul(L_t, K_t_inv)
...
def camarillo_constant_curvature_slack(delta_lengths: List[Tuple[float, ...]],
                                          cable_positions: List[Tuple[Tuple[float, ...], ...]],
                                          segment_stiffness_vals: List[Tuple[float, ...]],
                                          cable_stiffness_vals: List[Tuple[float, ...]],
                                          segment_lengths: List[float],
    ...
    def grad_f(delta: np.ndarray, out: np.ndarray) -> None:
        out[()] = np.matmul(np.transpose(C_m_inv) + C_m_inv, delta) + np.matmul(C_m_inv, y_hat)
    ...
    def h(delta: np.ndarray, lagrange: np.ndarray, obj_factor: float, out: np.ndarray) -> np.ndarray:
        out[()] = obj_factor * hessian_matrix[hessian_matrix != 0]

    # Set up slack estimation optimization problem
    nlp = ipyopt.Problem(n=len(y_hat), 
...
print(camarillo_constant_curvature_slack(delta_lengths, cable_positions, segment_stiffness_vals, cable_stiffness_vals, segment_lengths))
