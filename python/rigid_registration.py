...
# Parameters
REG_FILE = "../data/regs/reg_04_22_24b.csv"
...
tip_meas_in_aurora = meas_in_aurora[:, num_model_points:num_model_points + num_tip_points]

# Perform registrations
_, T_aurora_2_model, rmse["aurora_2_model"] = kinematics.rigid_align_svd(model_meas_in_aurora, model_truth_in_model)
_, T_aurora_2_tip, rmse["aurora_2_tip"] = kinematics.rigid_align_svd(tip_meas_in_aurora, tip_truth_in_tip)

# Get average coil transform
coil_qs = np.concatenate([transform[0] for transform in aurora_transforms['0B']], axis=1)
coil_ts = np.concatenate([transform[1] for transform in aurora_transforms['0B']], axis=1)
...
if output:
    np.savetxt("../tools/T_aurora_2_model", T_aurora_2_model)
    np.savetxt("../tools/T_tip_2_coil", T_tip_2_coil)
