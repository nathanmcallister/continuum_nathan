...
combined_pos = np.ma.compress_cols(np.ma.concatenate((phi_meas_pos, theta_meas_pos), axis=1))
combined_tang = np.ma.compress_cols(np.ma.concatenate((phi_meas_tang, theta_meas_tang), axis=1))
combined_dls = np.ma.compress_cols(np.ma.concatenate((phi_meas_dls, theta_meas_dls), axis=1))
...
for i in range(theta_steps * phi_steps):
    ...
    phi_webster_params, _ = mike_cc.one_seg_forward_kinematics(phi_dl[0] + xf[3], phi_dl[1] + xf[4], cable_positions[0], cable_positions[1], xf[2])
    T_phi = utils_cc.calculate_transform(phi_webster_params)
    phi_model_pos = T_phi[0:3, 3] + np.array([xf[0],xf[1],0])

    phi_avg_error[i] = np.sqrt(((phi_meas_pos[:, 5*i:5*(i+1)] - phi_model_pos.reshape((-1, 1)))**2).sum(axis=0).mean())

    theta_dl = theta_meas_dls[:, 5*i:5*(i+1)].mean(axis=1)
    theta_webster_params, _ = mike_cc.one_seg_forward_kinematics(theta_dl[0] + xf[3], theta_dl[1] + xf[4], cable_positions[0], cable_positions[1], xf[2])
    T_theta = utils_cc.calculate_transform(theta_webster_params)
    theta_model_pos = T_theta[0:3, 3] + np.array([xf[0],xf[1],0])

    theta_avg_error[i] = np.sqrt(((theta_meas_pos[:, 5*i:5*(i+1)] - theta_model_pos.reshape((-1, 1)))**2).sum(axis=0).mean())

plt.figure()
plt.contourf(180/np.pi*phi, 180/np.pi*theta, 0.5 * (phi_avg_error.reshape((theta_steps, phi_steps)) + theta_avg_error.reshape((phi_steps, theta_steps)).transpose()))
plt.colorbar()
