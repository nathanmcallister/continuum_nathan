...
def generate_babble_data(
    ...
    noisy_orientations = orientations + np.random.normal(0, tang_noise_std, orientations.shape)

    pure_container = utils_data.DataContainer()
    pure_container.from_raw_data(date, time, num_cables, num_measurements, cable_deltas, positions, orientations)
    pure_container.prefix = f"output/clean_{num_segments}_seg"

    noisy_container = utils_data.DataContainer()
    noisy_container.from_raw_data(date, time, num_cables, num_measurements, cable_deltas, noisy_positions, noisy_orientations)
    noisy_container.prefix = f"output/noisy_{num_segments}_seg"
...
if __name__ == "__main__":
    ...
    segment_stiffness_vals_two_seg = segment_stiffness_vals_one_seg + segment_stiffness_vals_one_seg
    cable_stiffness_vals_two_seg = cable_stiffness_vals_one_seg + cable_stiffness_vals_one_seg
    segment_lengths_two_seg = segment_lengths_one_seg + segment_lengths_one_seg

    pure_container_one_seg, noisy_container_one_seg = generate_babble_data(cable_positions_one_seg, segment_stiffness_vals_one_seg, cable_stiffness_vals_one_seg, segment_lengths_one_seg)

    pure_container_two_seg, noisy_container_two_seg = generate_babble_data(cable_positions_two_seg, segment_stiffness_vals_two_seg, cable_stiffness_vals_two_seg, segment_lengths_two_seg)
